...
void Board::init()   //根据维度初始化所有board[i][j]
{
...
QVector<int> Board::freePosition()//随机返回一个没有数字的tile的（i,j）位置
{
    QVector<int> pos;
    if (full()) {
        pos.append(-1);
...
void Board::move(Direction direction)
{
    ...
    notifyObservers(); //通知所有observers
}

void Board::prepareForNextMove()      // 为下次移动做准备，上轮得分0，上轮碰撞false
{
    tileCollisionLastRound = false;
    pointsScoredLastRound = 0;
    for (int i = 0; i < dimension; ++i) {
...
void Board::moveHorizontally(int i, int j, Direction dir)//水平移动
{
    if (board[i][j] != NULL) {
...
//垂直移动（出界，碰撞平）
void Board::moveVertically(int i, int j, Direction dir)//垂直方向进行移动
{
    if (board[i][j] != NULL) {
        bool tileCollision = false;
        int newi;
        ...
        // ... collision
            ...
            if (board[newi][j]->getValue() == board[i][j]->getValue() &&
                !board[newi][j]->getUpgratedThisMove()) {   //两值相等，没有升级
                tileCollision = true;
...
{
    board[i][j]->upgrade();//*2
    board[i][j]->setUpgratedThisMove(true);//升级
    pointsScoredLastRound += board[i][j]->getValue();//本局获得分数增加
}
